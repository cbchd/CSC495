...
public class StoryTimeServiceImpl extends RemoteEventServiceServlet implements StoryTimeService {
	...
	public Boolean loginUser(String username, String password) {
		...
		if (engine.loginUser(username, password)) {
			...
			if (DEBUG)
				System.out.println("Server: Fired Update Lobby Users Event for user: " + username);
			return true;
		} else {
			return false;
		}
	...
	public void hostLobbyRoom(String roomName, String theme) {
		...
		if (DEBUG)
			System.out.println("Server: Hosted room: " + roomName + ", with a theme of: " + theme
	...
	public void joinRoom(String roomName) {
		...
		for (User usr : engine.currentOnlineUsers) {
			if (usr.username.equalsIgnoreCase(user.username)) {
				if (DEBUG)
					System.out.println("Server: Found user: " + usr.username + ". Trying to add him to the room: "
							+ roomName);
				foundUser = usr;
				for (Room r : engine.currentLobbyRooms) {
					if (r.roomName.equalsIgnoreCase(roomName)) {
						...
						usr.room = r;
						if (DEBUG)
							System.out.println("Server: Added user: " + usr.username + " to the room: " + r.roomName);
						break;
					}
				}
			}
	...
	public LobbyRoomData getLobbyRoomInformation() {
		...
		for (User userReal : engine.currentOnlineUsers) {
			if (user.username.equalsIgnoreCase(userReal.username)) {
				...
				foundUser = true;
				if (DEBUG)
					System.out.println("Server: Found user " + user.username
		...
		if (room == null) {
			if (DEBUG)
				System.out.println("Server: User : " + user.username
						+ " has no room... so the client shouldn't be calling getlobbyroominformation");
		} else {
			if (DEBUG)
				System.out.println("Server: Got " + user.username + "'s room: " + room.roomName);
		}
		...
		if (DEBUG)
			System.out.println("Server: Adding users to the room info to be sent to user " + user.username);
		for (User usr : room.users) {
			// Add the relevant users to the room info to be sent
			roomInfo.users.add(usr.username);
			if (DEBUG)
				System.out.println("Server: Added user: " + usr.username + " to the roomInfo list");
		}
	...
	public void updateLobbyRoomPointCap(String roomName, int pointCap) {
		for (Room r : engine.currentLobbyRooms) {
			if (r.roomName.equalsIgnoreCase(roomName)) {
				r.pointCap = pointCap;
				if (DEBUG)
					System.out.println("Server: Set " + r.roomName + "'s pointCap to " + r.pointCap);
				...
				if (DEBUG)
					System.out.println("Server: Fired Point Change Event for domain: "
							+ DomainFactory.getDomain(roomName).getName());
				return;
			}
		}
	}
	...
	public void updateLobbyRoomTimer(String roomName, int timer) {
		if (DEBUG)
			System.out.println("Server: Got UpdateTimerEvent for room: " + roomName + " and updated timer value: "
					+ timer);
		for (Room r : engine.currentLobbyRooms) {
			if (r.roomName.equalsIgnoreCase(roomName)) {
				r.timer = timer;
				...
				if (DEBUG)
					System.out.println("Server: Fired UpdateTimerEvent for domain "
							+ DomainFactory.getDomain(roomName).getName());
				return;
			}
		}
	...
	public void leaveRoom(String roomName) {
		...
		if (usersRoom.host.getUsername().equalsIgnoreCase(foundUser.username)) {
			...
			if (DEBUG)
				System.out.println("Server: User " + usr.username + " was the host of " + roomName
			...
			if (DEBUG)
				System.out.println("Server: Fired user left event for domain " + roomName);
			return;
		}
	}
	...
	public void startGame(String roomName) {
		...
		if (DEBUG)
			System.out.println("Server: Trying to start game: Removed room: " + room.roomName);
		...
		if (DEBUG)
			System.out.println("Server: Started game with attributes: Domain: " + gameRoom.domain + ", pointCap: "
					+ gameRoom.pointCap + ", theme: " + gameRoom.theme + ", and timer: " + gameRoom.timer);
	...
	public String getMyUsername() {
		...
		User usr = (User) session.getAttribute("User");
		if (DEBUG)
			System.out.println("Server: Sending client's username: " + usr.username + " to client");
		return usr.username;
	}
	...
	public String getStartGameChooser(String roomName) {
		...
		for (InGameRoom r : engine.inGameRooms) {
			if (r.domain.getName().equalsIgnoreCase(roomName)) {
				...
				if (DEBUG)
					System.out.println("Server: Found the first chooser " + username);
			}
		}
		if (DEBUG)
			System.out.println("Server: Sent client the first chooser: " + username); // domain.getName()
																						// is
																						// null
																						// //TODO
		return username;
	...
	public void setReady(String username, String roomName) {
		...
		for (InGameRoom ingameRoom : engine.inGameRooms) {
			if (DEBUG)
				System.out.println("Server: Room: " + ingameRoom.domain.getName());
			if (ingameRoom.domain.getName().equals(roomName)) {
				if (DEBUG)
					System.out.println("Server: Found user's room: " + roomName + " in the current ingameRooms list");
				for (User user : ingameRoom.users) {
					...
					if (user.username.equalsIgnoreCase(username)) {
						...
						numberOfReady++;
						if (DEBUG)
							System.out.println("Server: Client: " + username + " has his status set to 'ready'");
				...
				if (numberOfReady == ingameRoom.users.size()) {
					...
					if (DEBUG)
						System.out.println("Server: Set the first rounds chooser to: " + roundEvent.chooser);
					addEvent(ingameRoom.domain, roundEvent);
					if (DEBUG)
						System.out.println("Server: Fired Initial Round Start Event with chooser: "
								+ roundEvent.chooser);
				}
			}
		}
	...
	public void submitPhrase(String phrase, String roomName) {
		...
		for (InGameRoom r : engine.inGameRooms) {
			if (r.domain.getName().equals(roomName)) {
				...
				for (User user : r.users) {
					if (user.username.equals(usr.username)) {
						...
						if (DEBUG)
							System.out.println("Server: Fired Phrase Submitted Event for phrase " + phrase);
						r.phrasesSubmitted++;
					}
				}
			}
			if (r.phrasesSubmitted == r.users.size()) {
				...
				if (DEBUG)
					System.out.println("Server: All users in room: " + r.domain.getName()
							+ " have submitted phrases for this round.");
				if (DEBUG)
					System.out.println("Server: Fired All Phrases Submitted Event");
			...
			if (r.domain.getName().equalsIgnoreCase(roomName)) {
				...
				for (User user : r.users) {
					if (user.phrase.equalsIgnoreCase(phrase)) {
						user.score++;
						if (DEBUG)
							System.out.println("Server: " + user.username + "'s word was chosen, his score is now: "
									+ user.score);
						if (user.score == r.pointCap) {
							...
							if (DEBUG)
								System.out.println("Server: User: " + user.username + " has hit the point limit.");
							if (DEBUG)
								System.out.println("Server: Fired GameEndEvent with winner: " + user.username);
							r.gameEnded = true;
						}
					}
				...
				if (r.gameEnded)
					return;
				if (DEBUG)
					System.out.println("Server: Fired Round Close Event for room: " + roomName);
				...
				if (DEBUG)
					System.out.println("Server: Fired Round Start Event for room: " + roomName);
			}
		}
	}
