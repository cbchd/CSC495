...
public class GameInProgressRoom {
	...
	storyTimeService.getStartGameChooser(gameData.domain.getName(),
		new AsyncCallback<String>() {
			...
			if (DEBUG)
			    System.out
				    .println("Got & set the first phrase chooser: "
					    + result);
	...
	btnChoosePhrase.addClickHandler(new ClickHandler() {
		...
		storyTimeService.choosePhrase(chosenPhrase,
			gameData.domain.getName(), new AsyncCallback<Void>() {
	...
	theRemoteEventService.addListener(gameData.domain,
		new RemoteEventListener() {
			...
			if (anEvent instanceof RoundStartEvent) {
			    if (DEBUG)
				System.out
					.println("Client: Got Round Start Event");
			    ...
			    if (DEBUG)
				System.out
					.println("Client: Got current chooser: "
						+ gameData.currentChooser);
			    if (gameData.currentChooser
				    .equalsIgnoreCase(username)) {
				if (DEBUG)
				    System.out
					    .println("Client: This user ("
						    + username
			    ...
			    if (choosing) btnChoosePhrase.setVisible(true);
			    if (DEBUG)
				System.out
					.println("Got Phrase Submitted Event for phrase "
						+ phraseEvent.phrase);
			    ...
			    if (DEBUG)
				System.out
					.println("Client: Recieved Phrase Chosen Event");
			    ...
			    if (DEBUG)
				System.out
					.println("Client: Got Round Close Event");
			    for (String user : roundCloseEvent.pointList
				    .keySet()) {
				if (user.equalsIgnoreCase(username)) {
				    // update this user's score display
				    // score
				    lblScorebox.setText(""
					    + roundCloseEvent.pointList
						    .get(user));
				    if (DEBUG)
					System.out
						.println("Updated user's current score to: "
							+ roundCloseEvent.pointList
								.get(user));
				}
				if (DEBUG)
			    ...
			    if (DEBUG) System.out.println(gameData.winner + " has won the game!");
	...
	Timer timer = new Timer() {
		...
		if (DEBUG)
		    System.out
			    .println("Setting the client as ready.. after 2 seconds");
		this.cancel();
	    }
	};
	timer.schedule(3000);
